...
if (mysqli_num_rows($result1)) { 
	$_SESSION['pesan_register_nasabah'] = $_SESSION['pesan_register_nasabah'] . $i . '. Username Sudah Terpakai';
	$i++;
	$_SESSION['username_register_nasabah'] = $username;
	$_SESSION['password_register_nasabah'] = $password;
	$_SESSION['email_register_nasabah'] = $email;
}

$result2 = mysqli_query($conn, "SELECT * FROM user WHERE email = '$email'");
if (!$result2) { die("SQL ERROR : result2"); }
if (mysqli_num_rows($result2)) { 
	$_SESSION['pesan_register_nasabah'] = $_SESSION['pesan_register_nasabah'] . $i . '. E-Mail Sudah Terpakai';
	$i++;
	$_SESSION['username_register_nasabah'] = $username;
	$_SESSION['password_register_nasabah'] = $password;
	$_SESSION['email_register_nasabah'] = $email;
...
if($i == 1){
	...
	$result5 = mysqli_query($conn, "INSERT INTO nasabah(nama, alamat, telp, noHp, noKtp, namaPekerjaan, alamatPekerjaan, User_id) VALUES ('', '', '', '', '', '', '', $id)");
